...
data_destination = "/Users/rzieber/Documents/3D-PAWS/Turkiye/debug/Adana_Complete/"
station_directories = [
...
for i in range(len(station_files)):
    ...
    file_name = station_directories[i][8:len(station_directories[i])-1] + "_Complete_Record.dat"
    temp_file_name = file_name[:len(file_name)-4] + "_TEMP.dat"
    destination = os.path.join(data_destination, file_name)
    temp_file_path = os.path.join(data_destination, temp_file_name)
